...
最終更新: 2026-07-15（v2.3-b4t なかまわけ拡張＋feature-spec追随＋台帳分割・実機確認待ち）
...
- **設計書の版**: `roadmap.md`・`feature-spec.md` は b4t 時点へ追随済み（2026-07-15）
...
1. **実機で継続確認**（大きな刷新が続いたため・各版の⚠️次を参照）: 相棒/たまごサイクル（孵化テンポ `EGG_HATCH_XP=40`・b4s卵欄）・結果シーケンスのタメ（SEQ定数）・バトル入口背景・タイピングのタップ入力・なかまわけ新形式（b4t）ほか b4系一式
