...
> 過去の版ごとの詳細ログは月別アーカイブへ（読むのは必要なときだけ）: **v2.3-b4e〜b6g** = `progland-archive-2026-07.md`（版のログのほか、**台帳から移した完了項目**もここ＝開店フェーズ確定事項／ゲームこうぼう段階A〜3の記録／完了フェーズの指示書／引退した未完了タスク） ／ **v2.3-b4d 以前**（＋過去フェーズの教訓の詳細）= `progland-handoff-archive.md`。

> ### ★台帳の維持規則（2026-07-24 制定／2026-07-25 改訂・全セッション厳守）
> **「今どこか」の版エントリは直近8版まで。** 9版目を足すときは、**最も古い1版を `progland-archive-YYYY-MM.md` へ移してから**追記する（原文のまま・要約しない。アーカイブは月ごとに分ける）。
>
...
> - **9版以上** → 最も古い版をアーカイブへ**原文のまま**移す
> - **8版以下なのに超過** → 版ではなく**固定部（未完了タスク・生きている注意事項・ファイルの地図・決定事項）が育っている**。**完了・解決した項目を消すか、アーカイブへ移す**。生きている項目には手を付けない
> - **8版以下で、固定部にも消せる項目が無い** → 規則は守られている。重い版が続いただけ。**何もしない**
>
