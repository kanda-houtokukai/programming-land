...
- **フェーズ: P3完了＋追補（メイン絵の画像化 v2.0-P3.2）公開済み。ユーザーの実機確認待ち。次は P4（タイピング）。**
...
1. ユーザーが公開URL（v2.0-P3.2）を実機確認（PC Safari＋タブレット。特に盤面ロボットのドット絵が iPad でくっきり出るか）
2. OKなら P4「タイピング」: 3段階・ローマ字は訓令式/ヘボン式両対応・変換テーブル単体テスト必須
