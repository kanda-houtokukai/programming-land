...
export function newProfileData(name = "", character = null) {
  return {
    ...
    gamelab: { works: [], draft: null, milestones: {} }, // ゲームこうぼう: works=保存ゲーム / draft=かきかけ / milestones=コイン節目の達成フラグ（便③・★studio.milestones と別の場所＝達成が相互汚染しない）
  };
}
...
// App の外の画面（#studio-dev 等）から「今の子」を取る: 最後に遊んだ子→いなければ先頭→いなければ null
export function lastProfile() {
  const m = loadMeta();
  return (m.lastProfileId && loadProfile(m.lastProfileId)) || listProfiles()[0] || null;
}
