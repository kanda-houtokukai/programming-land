...
const TRIGGERS = ["hat", "tap", "bump", "bumpTarget"]; // 同拍競合の処理順もこの順（bumpTarget=stage2・相手指定ぶつかり）
...
export function createEngine(charDefs, cb = {}) {
  ...
  // 相手指定ぶつかり（stage2）: self が bumpTarget を持ち、指定相手（"any" or 相手key）と重なったら発火
  const fireBumpTarget = (self, other) => {
    const t = self.triggers.bumpTarget;
    if (!t) return;
    const tgt = t.head.target;
    if (tgt == null || tgt === "any" || tgt === other.key) startThread(self, "bumpTarget");
  };

  /* ---- 公開API ---- */
  const hasAnyTrigger = () => chars.some(c => TRIGGERS.some(k => c.triggers[k]));
  const hasListeners = () => chars.some(c => c.triggers.tap || c.triggers.bump || c.triggers.bumpTarget);
    ...
    tick() {
      ...
      for (const pk of now) {
        if (!prevOverlap.has(pk)) {
          ...
          startThread(a, "bump");
          startThread(b, "bump");
          fireBumpTarget(a, b); fireBumpTarget(b, a); // stage2: 相手指定ぶつかり
        }
      }
      prevOverlap = now;
