...
export function createEngine(charDefs, cb = {}) {
  ...
  const chaseTarget = (self, target) => {
    if (target && target !== "any") { const t = byKey.get(target); if (t && t.visible) return t; }
  ...
  const beginBlock = (th, b) => {
    ...
    if (b.type === "moveRand") { // stage2: 範囲内の4方向からランダムに1マス（塞がれていればその場）
      const cand = [[1, 0], [-1, 0], [0, 1], [0, -1]].filter(([dx, dy]) => {
        const nx = ch.x + dx, ny = ch.y + dy; return nx >= 0 && nx < LCOLS && ny >= 0 && ny < LROWS;
      });
      if (cand.length) { const [dx, dy] = cand[Math.floor(Math.random() * cand.length)]; ch.x += dx; ch.y += dy; emitUpdate(ch, "move"); }
      th.cur = { b, left: 0 }; return;
    }
    if (b.type === "bounce") { // stage2: 向きへ1マス・端で反転（初期向き=右）。ずっとと組むと往復
      if (!ch.dir) ch.dir = [1, 0];
      let [dx, dy] = ch.dir;
      let nx = ch.x + dx, ny = ch.y + dy;
      if (nx < 0 || nx >= LCOLS || ny < 0 || ny >= LROWS) { ch.dir = [-dx, -dy]; nx = ch.x - dx; ny = ch.y - dy; emitFx(ch.key, { type: "bump" }); }
      if (nx >= 0 && nx < LCOLS && ny >= 0 && ny < LROWS) { ch.x = nx; ch.y = ny; emitUpdate(ch, "move"); } // 反転後も外なら留まる
      th.cur = { b, left: 0 }; return;
    }
    if (b.type === "chase") { // stage3: 指定相手へ1拍1マス寄る（差の大きい軸へ）。同マスなら寄らない＝重なって暴れない
      const tgt = chaseTarget(ch, b.target);
      if (tgt) {
        const ddx = tgt.x - ch.x, ddy = tgt.y - ch.y;
        if (ddx !== 0 || ddy !== 0) {
          const [dx, dy] = Math.abs(ddx) >= Math.abs(ddy) ? [Math.sign(ddx), 0] : [0, Math.sign(ddy)];
          const nx = ch.x + dx, ny = ch.y + dy;
          if (nx >= 0 && nx < LCOLS && ny >= 0 && ny < LROWS) { ch.x = nx; ch.y = ny; emitUpdate(ch, "move"); }
        }
      }
      th.cur = { b, left: 0 }; return;
    }
    if (b.type === "fall") { // stage3: 1拍1マス下。下端に着いたら上端へ戻る（横位置はランダム＝同じ列に落ち続けない）
      if (ch.y <= 0) { ch.y = LROWS - 1; ch.x = Math.floor(Math.random() * LCOLS); }
      else ch.y -= 1;
      emitUpdate(ch, "move");
      th.cur = { b, left: 0 }; return;
    }
  ...
  const fireTargetTrigger = (self, other, kind) => {
    ...
    if (!t) return;
    const tgt = t.head.target;
    if (tgt == null || tgt === "any" || tgt === other.key) startThread(self, kind);
  };

  /* ---- 公開API ---- */
  const hasAnyTrigger = () => chars.some(c => TRIGGERS.some(k => c.triggers[k]));
  const hasListeners = () => chars.some(c => c.triggers.tap || c.triggers.bump || c.triggers.bumpTarget || c.triggers.goal);
