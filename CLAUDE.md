...
- ⚠️ 台帳に版を追記するときは**「今どこか」の版エントリを直近8版に保つ**（9版目を足す前に、最も古い1版を `progland-archive-YYYY-MM.md` へ原文のまま移す）。全セッションが起動時に読むため肥大はコンテキストの浪費。
  - **文字数は必ず `LC_ALL=ja_JP.UTF-8 wc -m progland-handoff.md` で測る。** ⚠️ 素の `wc -m` は使わない（この環境は locale が `C` のためバイト数を返し、日本語では**約2倍**に出て誤判定する）。
  - 台帳が **3.5万文字**を超えたら、まず版エントリ数を数える → **9版以上**なら最も古い版をアーカイブへ原文のまま移す／**8版以下なのに超過**なら**固定部（未完了タスク・生きている注意事項・ファイルの地図・決定事項）が育っている**ので、完了・解決した項目を消すかアーカイブへ移す（生きている項目は残す）／**8版以下で固定部にも消せる項目が無い**なら規則は守られている（重い版が続いただけ・何もしない）。
